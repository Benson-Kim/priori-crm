...
function Column({
    ...
    return (
        <div className={isLast ? "px-4 py-3" : "border-desk-border px-4 py-3 lg:border-r"}>
            <p className="text-[10px] font-bold tracking-[0.25px] text-desk-muted uppercase">
                {label}
            </p>
            <p className="pt-1 text-[13px] font-bold text-desk-ink">{headline}</p>
            <p className="pt-0.5 text-[11px] text-desk-muted">{caption}</p>
        </div>
    );
}
...
export function StageSummaryStrip({ overview }: Readonly<StageSummaryStripProps>) {
    ...
    return (
        <div className="grid grid-cols-1 divide-y divide-desk-border rounded-2xl border border-desk-border bg-desk-surface sm:grid-cols-2 sm:divide-y-0 lg:grid-cols-5">
            {stages.map((column) => (
                <Column
                    key={column.stage}
                    label={column.stage}
