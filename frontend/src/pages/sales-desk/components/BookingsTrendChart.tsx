...
function BookingsTooltip({ active, payload, currency }: Readonly<TooltipProps>) {
    ...
    return (
        <div className="rounded-xl border border-desk-border bg-desk-surface px-3 py-2 text-xs shadow-[0_4px_15px_rgba(0,0,0,0.08)]">
            <p className="font-semibold text-desk-ink">{month}</p>
            <p className="text-desk-green">Won {formatDeskMoney(won, currency)}</p>
            <p className="text-desk-red">Lost {formatDeskMoney(lostMagnitude, currency)}</p>
        </div>
    );
}
        ...
        <div>
            ...
            <div className="flex items-center gap-4 pt-3">
                <span className="flex items-center gap-1.5 text-xs text-desk-muted">
                    <span className="size-2.5 rounded-[3px] bg-desk-green" aria-hidden="true" />
                    Won
                </span>
                <span className="flex items-center gap-1.5 text-xs text-desk-muted">
                    <span
                        className="size-2.5 rounded-[3px] bg-desk-red opacity-45"
                        aria-hidden="true"
                    />
                    Lost
                </span>
