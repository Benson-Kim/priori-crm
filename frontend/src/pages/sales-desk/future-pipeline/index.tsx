...
function ProspectCard({ prospect }: Readonly<{ prospect: ProspectRow }>) {
    return (
        <Link
            ...
            className="flex flex-col rounded-2xl border border-desk-border bg-desk-surface p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] transition-colors hover:border-desk-muted"
        >
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    <h2 className="text-[15px] font-bold text-desk-ink">{prospect.company}</h2>
                    <p className="pt-0.5 text-xs text-desk-muted">{prospect.contact}</p>
                </div>
                <DueBadge prospect={prospect} />
            </div>

            <p className="py-4 text-[13px] leading-relaxed text-desk-ink">{prospect.note}</p>

            <div className="mt-auto flex items-end justify-between gap-3 border-t border-desk-border pt-3">
                <span className="flex items-center gap-2">
                    <RepAvatar
                        initials={prospect.owner_initials}
                        color={prospect.owner_color}
                    />
                    <span className="text-[13px] text-desk-ink">{prospect.owner_name}</span>
                </span>
                <span className="text-right">
                    <span className="block text-[11px] text-desk-muted">Est. ARR</span>
                    <span className="block text-[15px] font-bold text-desk-ink">
                        {formatDeskMoney(prospect.estimated_arr)}
                    </span>
                </span>
            </div>

            <p className="flex items-center gap-1.5 pt-3 text-xs text-desk-muted">
