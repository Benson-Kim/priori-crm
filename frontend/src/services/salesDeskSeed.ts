 ...
 * Sales Desk fixtures.
 *
 * There is no deals backend in `develop` yet, so no deals, stages, reps or
 * bookings endpoints exist. These fixtures are a worked pipeline transcribed
 * into typed records, so the screens can be built against real-shaped data.
 *
 ...
 * it too. A deal's `stage` is an index into DEAL_STAGES.
 */
import {
    DEAL_STAGES,
    type DealStage,
...
export const SEED_DEALS: SeedDeal[] = [
    {
        ...
        history: [
            ...
            {
                stage: "Qualification",
                note: "Budget confirmed with CFO. Security is the driver, needs Intune and Defender.",
                daysAgo: 12,
            },
            {
