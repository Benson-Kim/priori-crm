...
export function QuoteForm({
  ...
  const handleSubmit = async () => {
    ...
    const validItems = lineItems.filter(
      (r) => r.itemName.trim() || r.description.trim()
    );
    ...
    for (const item of validItems) {
      ...
      if (!item.itemName)
        newErrors[`item_${item.key}_name`] = "Item name is required";
      if (isNaN(qty) || qty <= 0)
      ...
      <div className="bg-white rounded-[20px] border-2 border-purple-25 overflow-hidden shadow-sm">
        ...
        <div className="p-8 grid grid-cols-2 md:grid-cols-5 gap-6">
          <div className="flex flex-col gap-2">
            <label className="text-[13px] text-gray-500 font-medium w-full">
              Reference
            </label>
            <p className="text-[16px] font-bold text-gray-800">
              {initialData?.quote_reference || "Autogenerated"}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2 relative w-full">
              <span className="text-[16px] font-bold text-priori-purple pr-8 whitespace-nowrap">
                {formatDisplayDate(transactionDate)}
              </span>
              {!restrictedMode && (
                <SquarePen
                  size={20}
                  className="text-priori-purple absolute right-0 pointer-events-none"
                />
              )}
              {!restrictedMode && (
                <input
          ...
          <div className="flex flex-col gap-2 relative group">
            ...
            <div className="flex items-center gap-2 relative w-full">
              <span className="text-[16px] font-bold text-priori-purple pr-8">
                {currency}
              </span>
              {!restrictedMode && (
                <SquarePen
        ...
        <div className="p-8 pb-4">
          <h3 className="text-[17px] font-bold text-gray-800 mb-5">
            Item Details
          </h3>
          {errors.lineItems && (
            ...
            <table className="w-full text-[16px]">
              <thead>
                <tr className="bg-priori-purple text-white">
                  <th className="text-left px-4 py-4 font-semibold w-[20%]">
                    Item
                  </th>
                  <th className="text-left px-4 py-4 font-semibold w-[40%]">
                    Description
                  </th>
                  <th className="text-left px-4 py-4 font-semibold w-[15%]">
                    Quantity
                  </th>
                  <th className="text-left px-4 py-4 font-semibold w-[15%]">
                    Price
                  </th>
                  <th className="text-left px-4 py-4 font-semibold w-[10%]">
                    Total
                  </th>
                </tr>
              </thead>
              <tbody>
                {lineItems.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.key}
                      className="border-b border-gray-300 hover:bg-gray-100 transition-colors group relative"
                    >
