...
export function DocumentEditor({
    ...
    const handleSubmit = async () => {
        ...
        const validItems = lineItems.filter((r) => r.description.trim() || r.itemName.trim());
        if (validItems.length === 0) newErrors.lineItems = "At least one line item is required";

        for (const item of validItems) {
            const qty = parseFloat(item.quantity);
            const price = parseFloat(item.unitPrice);
            if (!item.itemName.trim() && !isInvoice) newErrors[`item_${item.key}_name`] = "Item name is required";
                ...
                <div className="p-6 gap-6">
                    <div className="grid grid-cols-2 md:grid-cols-10 gap-6">
                        <div className="flex flex-col gap-2 md:col-span-2">
                            ...
                            <p className="font-bold text-gray-800">{initialData?.documentReference || "Autogenerated"}</p>
                        </div>
                                        ...
                                        <Fragment key={row.key}>
                                            <tr className="group relative mb-3 grid grid-cols-7 gap-4 pt-2">
                                                <td className="align-left col-span-2">
                                                    <input
                                                        type="text"
                                                        value={row.itemName || row.description.split('\n')[0] || ""}
                                                        onChange={(e) => updateRow(row.key, "itemName", e.target.value)}
                                                        placeholder="Item name"
                                                        disabled={restrictedMode}
                                                        className={`w-full bg-transparent px-3 py-4 border ${errors[`item_${row.key}_name`] ? 'border-red-500' : 'border-gray-300'} font-bold text-gray-800 placeholder-gray-300 rounded-lg`}
                                                    />
                                                </td>
