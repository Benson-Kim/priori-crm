...
def _make_service(monkeypatch, po: _FakePO):
    """Return (service, payments_dict) with DB interactions monkeypatched."""
    db = MagicMock()
...
class TestFxResyncMultiPayment:
    ...
    def test_fx_resync_multi_payment_exact(self, monkeypatch):
        ...
        payment_inputs = [
            (Decimal("30.00"), Decimal("129.75")),
            (Decimal("20.00"), Decimal("130.33")),
            (Decimal("25.00"), Decimal("128.50")),
        ]
        ...
        expected_total = sum(
            PurchaseOrderService._converted_amount(a, r) for a, r in payment_inputs
        )
