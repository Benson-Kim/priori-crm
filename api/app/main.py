...
def create_app() -> FastAPI:
    ...
    # added is the OUTERMOST. CORSMiddleware MUST be registered last so it
    # wraps every other middleware. Otherwise a response produced by an
    # outer middleware (e.g. the rate limiter's short-circuit 429) never
    # passes through the CORS layer and ships without
    # Access-Control-Allow-Origin, which the browser reports as a generic
    # "network error" instead of the real status code.
    #
    # Inner ordering (registered first => innermost):
    #   1. RateLimitMiddleware   - innermost custom layer; its 429 unwinds
    #                              back out through the layers below.
    #   2. RequestLoggingMiddleware
    #   3. RequestIDMiddleware
    #   4. SecurityHeadersMiddleware
    #   5. CORSMiddleware        - OUTERMOST; CORS headers on every response.
    ...
    # Security headers apply to every response, including error and
    # throttled (429) paths (still inside CORS, added below).
    ...
    # Registered LAST => outermost middleware, so CORS headers are present on
    # every response, including short-circuited 429s and error responses
    # generated by inner middleware.
