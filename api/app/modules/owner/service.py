...
class OwnerService:
    ...
    def _schedule_object_cleanup(self, *keys: str | None) -> None:
        ...
        Thin wrapper over the shared ``storage_tx`` helper: the service
        flushes but never commits (the request-scoped commit is owned by
        ``CommitOnSuccessRoute`` in ``app/common/routing.py``; ``get_db()``'s
        teardown commit is a safety net only), so the old object is only
        deleted once the outer
