...
Companion to ADR-0014 (the tenancy decision and phased blueprint); every
...
| # | Subsystem | Current state (citation) | What breaks at N>1 | Severity | Required change | ADR-0014 phase |
|---|-----------|--------------------------|--------------------|----------|-----------------|----------------|
...
| 10 | CORS / FRONTEND_BASE_URL / SES identity | Single global config: `CORS_ORIGINS`/regex (`api/app/lib/config.py:49-56`), `FRONTEND_BASE_URL` used in reset links (`config.py:70`, `auth/service.py::_send_password_reset_email`), one `SES_SENDER_EMAIL` (`config.py:45`) | Works for a shared-domain SaaS; breaks only if tenants get their own domains (wrong reset-link host, wrong sender). Shared SES identity means one tenant's bounce behaviour damages all tenants' deliverability | **Low** (shared domain) / **High** (per-tenant domains) | Keep platform-global for the shared-domain model (ADR-0014 decision); revisit per-tenant domains as a separate ADR; monitor SES reputation per runbook | T6 note; runbook |
