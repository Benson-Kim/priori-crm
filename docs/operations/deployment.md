...
> **The `deploy` job above now lives in `deploy-release.yml`** (`workflow_call`),
> called by `deploy-production.yml` with production values pinned and — with
> drill-only `DRILL_SSH_*` secrets and a guarded target — by `deploy-drill.yml`,
> so the quarterly DR drill exercises the same artifact-download → rsync →
> ops-script → release-script plumbing against a throwaway droplet (issue #87;
> drill procedure in the backup runbook's drill Variant A section). The excerpt
> shows the logical steps; the extracted files are authoritative.
