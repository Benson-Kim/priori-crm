...
log "     - deploy user's rclone remotes: base 'spaces' (droplet RW bucket key) +"
log "       'spaces-uploads-crypt' (crypt over <bucket>/uploads-sync) — fill"
log "       deploy/rclone-uploads-crypt.conf.template -> ~deploy/.config/rclone/rclone.conf (0600)"
